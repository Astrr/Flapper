...
def init(args, reinit)
  return unless reinit == 1 || args.state.not_init.zero?

  if args.state.not_init.zero?
    args.state.not_init = 1
  end
  args.state.pipes = []
...
def check_dead(args)
  return unless args.state.pipes.any? { |p| p.rect.intersect_rect?(args.state.player.rect) } or args.state.dead == 1
