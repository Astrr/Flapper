...
def gravity(args)
  ...
  if args.state.player_y + args.state.player_accel >= 620
    args.state.player_accel = -args.state.player_accel - 10
    if args.state.player_accel.positive?
      args.state.player_accel = 0
    end
    args.state.player_y = 620
  end
end

def move_player(args)
  args.state.player_y += args.state.player_accel
  args.state.player.h = 64 * ((1 - args.state.player_accel/50))
end
