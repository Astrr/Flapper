...
def generate_pipes(args)
  return unless args.state.pipes == []

  args.state.pipes << [1000, 0, 100, args.state.upper_height]
  args.state.pipes << [1000, 720, 100, -args.state.lower_height + 100]
end
...
def tick(args)
  args.state.pipes ||= []
  args.state.pipe_timer = 120
  args.state.pipe_timer -= 1
  if args.state.pipe_timer.iszero?
  generate_pipes(args)
  end
