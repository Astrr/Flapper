...
def tick(args)
  ...
  if args.state.pipes { |p| p.rect.intersect_rect?(args.state.player.rect) }.first
    if (args.state.player_y == args.state.pipes.any? { |p| p.h}) && args.state.dead == 1
      args.state.player_accel = 0
      args.state.gravity_coeff = 0
    end
  end
