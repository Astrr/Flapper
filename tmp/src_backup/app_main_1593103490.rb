...
def tick(args)
  ...
  args.state.player = {
    ...
    w: 64, # WIDTH
    h: 64, # HEIGHT
    path: 'sprites/wingMan1-scaled.png'
  }
  ...
  if args.inputs.mouse.click and args.state.dead == 1
    reset_game(args)
  end
  if (args.inputs.keyboard.key_down.space and args.state.dead == 0) or (args.inputs.mouse.click and args.state.dead == 0)
    jump args
    args.state.flap = 1
  end
  animate_player args
...
def player_ded(args)
  args.state.dead = 1
  args.outputs.labels << [640, 360, "U ded, press R or tap to retry", 10, 1, 255, 0, 0]
  args.outputs.background_color = [178, 143, 61]
end
...
def animate_player(args)
  ...
  if args.state.anim_in_progress
    ...
    when 0
      args.state.player.path = "sprites/wingMan2-scaled.png"
    when 1
      args.state.player.path = "sprites/wingMan3-scaled.png"
    when 2
      args.state.player.path = "sprites/wingMan4-scaled.png"
    when 3
      args.state.player.path = "sprites/wingMan5-scaled.png"
    when 4
      args.state.player.path = "sprites/wingMan1-scaled.png"
